...
function App() {
  ...
  const deleteNote = (entry) => {
    ...
    alert("You are about to delete a note, proceed?")
    try {
  ...
  const deleteAllNotes = () => {
    ...
    alert("You are about to delete all notes, proceed?")
    try {
  ...
  const deleteNoteState = (entry) => {
    ...
    setNotes((prevNotes) => prevNotes.filter((lstobj) => lstobj !== entry))
  }

  const deleteAllNotesState = () => {
    // Code for modifying state after DELETE all here
    setNotes((prevNotes) => prevNotes.filter((lstobj) => lstobj === null))
  }

  const patchNoteState = (_id, title, content) => {
    setNotes((prevNotes) => prevNotes.map(lstobj => {
        if(lstobj._id === _id){
        lstobj.title = title
        lstobj.content = content
      }
      return lstobj
    }))
  }
